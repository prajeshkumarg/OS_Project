...
#include<math.h>
using namespace std;
void filesys::set_file_system_name(){ // function to create a new file system name
    cout << "Enter your file system name\n";
    char temp[80];
    cin >> temp;
    strcpy(file_system_name, temp);
    initialize();
}
filesys::filesys(char f_name[]){ //function to check whether create a new file system or open existing one
if(f_name == NULL)
...
void filesys::initialize(){ // actual file system is created here
...
void filesys::list_files(){ //function to list files under the file system
    for(int i = 0; i < MAX_FILES; i++){
        if(!strcmp(files[i].get_file_name(), "\0"))  //check if end of file names
        break;
        cout << files[i].get_file_name() << endl;
    }
}
char* filesys::show_file_content(char* f_name){ //function to show file contents
...
char* filesys::search_file(char* f_name){ //function for search files
    int i;
    for(i = 0; i < MAX_FILES; i++){
    if(!strcmp(files[i].get_file_name(), f_name)){
    cout<<"File found\nFile name:";
...
void filesys :: search_keyword(char* f_name, char* keyword){ //function to find keyword
    fstream myfile(file_system_name, ios::in);
    for(int i = 0; i < MAX_FILES; i++){
...
void filesys::delete_file(char* f_name) //function to delete files
    {int i,j;
    char* file_content = new char[MAX_FILE_LEN];
    for(i = 0; i < MAX_FILES; i++){
        if(!strcmp(files[i].get_file_name(), f_name)){
...
void filesys::create_file(char* f_name, char* file_contents){ //create new file in the file system
    int i;
    for(i = 0; i < MAX_FILES; i++){
...
void filesys::read_from_file(){ //function to read from a file
    fstream myfile(file_system_name, ios::in);
    int i;
...
void filesys::write_to_file(){ //function to write to files
    fstream myfile(file_system_name, ios::in);
